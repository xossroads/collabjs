...
export type SandboxLogKind = 'log' | 'error' | 'warn' | 'info' | 'result';
...
export class Sandbox {
  ...
  private onLog: (msg: SandboxMessage) => void;
  private onDone: () => void;

  constructor(onLog: (msg: SandboxMessage) => void, onDone: () => void) {
    this.onLog = onLog;
    this.onDone = onDone;
  }
        ...
        case 'ready':
          if (this.pendingCode !== null && this.iframe.contentWindow) {
            this.iframe.contentWindow.postMessage(
              { kind: 'execute', code: this.pendingCode },
              '*'
            );
